...
* [docs/design.md](./design.md) holds the conceptual model and semantic mechanisms: what a program, goal, step, transaction, and certificate are; the core principle that the untrusted agent proposes while sound checkers certify; decomposition via outlining and step narrowing; interface strengthening; and the no-`undef` model.
* [docs/implementation.md](./implementation.md) holds the physical architecture and execution details: the TypeScript/C++ language split; toolchain layout and external checkers (alive-tv, llubi); state on disk under `sessions/<id>/` and `trajectory.jsonl`; the certificate package format and replay algorithm in `kernel/check.py`; the visualizer; configuration precedence; and build targets in Makefile.
