...
alive-tv is run with the SMT timeout and whatever flags the caller passes, and nothing else. That timeout bounds one query rather than the process, so a check has a wall clock of its own: the budget again, with at most half a minute of grace, so that a check asking for three seconds cannot cost thirty. A process that clock stops is reported as unknown rather than as an error, because a checker we killed says nothing about the pair, while a checker that failed says something about the toolchain, and the two send a reader to different places.

The one thing a caller does pass is what the run assumes about the pair's arguments, as `--disable-undef-input` and `--disable-poison-input`. It is an `ArgumentAssumption`, from `engine/core/state/arguments.ts`, and it is called `assumed` wherever it is recorded, which is never where the pair is: the programs are content-addressed and hashed, and this is one statement about the question. `Session.start` writes it into `run_start` and the goal tree derives it from there, so a resumed run and a certificate read the same statement rather than being told it again. Which goals it reaches is `hasRootEntry`: the root, and the outer half of every cut under it, never a callee.

Everything else about definedness is stated in the IR. A callee's interface is proved defined at the cut by strengthening, which certifies an assume in the caller before putting the same fact on the parameter, and a root pair may carry `noundef` on a parameter it takes to be neither undef nor poison, which is more than the run's assumption says.
